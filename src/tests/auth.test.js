...
async function runTests() {
    ...
    try {
        const configExists = fs.existsSync(path.join(projectRoot, 'services/firebase.ts'));
    ...
    try {
        const authContextExists = fs.existsSync(path.join(projectRoot, 'context/AuthContext.tsx'));
    ...
    try {
        const signInExists = fs.existsSync(path.join(projectRoot, 'pages/SignIn.tsx'));
    ...
    try {
        const signUpExists = fs.existsSync(path.join(projectRoot, 'pages/SignUp.tsx'));
    ...
    try {
        const protectedRouteExists = fs.existsSync(path.join(projectRoot, 'components/ProtectedRoute.tsx'));
    ...
    try {
        ...
        if (envExists) {
            ...
            test('Environment file contains API Key', hasApiKey);
        }
    } catch (e) {
